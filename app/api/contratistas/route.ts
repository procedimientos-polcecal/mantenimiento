...
// GET — lista (cualquier autenticado)
export async function GET() {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return NextResponse.json({ error: "No autorizado" }, { status: 401 });
  const admin = createAdminClient();
  const { data, error } = await admin.from("contratistas").select("*").order("nombre");
...
export async function POST(request: Request) {
  ...
  const admin = createAdminClient();
  const { error } = await admin.from("contratistas").insert({ nombre: nombre.trim() });
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
...
export async function DELETE(request: Request) {
  ...
  const admin = createAdminClient();
  const { error } = await admin.from("contratistas").delete().eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ success: true });
}

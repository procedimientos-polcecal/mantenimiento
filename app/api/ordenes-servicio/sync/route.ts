...
import { runOrdenesServicioSync } from "@/lib/sheets-sync";
...
export async function POST() {
  ...
  try {
    const synced = await runOrdenesServicioSync();
...
export async function GET() {
  ...
  const { data } = await admin.from("ordenes_servicio").select("synced_at")

...
import { runComparativasSync } from "@/lib/sheets-sync";

// POST — sincronización manual (admin): importa/refresca desde la planilla
export async function POST() {
  ...
  try {
    const synced = await runComparativasSync();
    return NextResponse.json({ synced });
  } catch (err: any) {
    console.error("Comparativas sync error:", err);
...
export async function GET() {
  ...
  const { data } = await admin.from("os_comparativas").select("synced_at")
